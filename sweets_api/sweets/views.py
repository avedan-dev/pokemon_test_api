...
# Проверяет остались ли все заказы доступными курьеру, после измениния его данных данных курьера. Если есть лишние,
# то удаляяются лишние заказы
def courier_check(courier):
    cour_orders = CouriersAndOrders.objects.filter(courier_id=courier.courier_id).values(
        "order_id")  # Получаю класс id заказов, записанных на курьера
    orders = Order.objects.filter(order_id__in=cour_orders)  # Получаю заказы по их id
...
class CouriersGetPatchView(APIView):
    def patch(self, request, pk):
        ...
        if serializer.is_valid(raise_exception=False):
            ...
            return Response(courier_filter_data(courier), status=status.HTTP_200_OK)
        else:
        ...
        if serializer.is_valid(raise_exception=False):
            ...
            for order in orders:
                ...
                for work_time in s:
                    for i in range(len(order.delivery_hours)):
                        order_time = str_to_time(order.delivery_hours[i])
                        if work_time[0] < order_time[0] < work_time[1] or order_time[0] < work_time[0] < order_time[1] \
                                or work_time[0] == order_time[0] and work_time[1] == order_time[1]:
            ...
            if ans:
                return Response({"orders": [{"id": ans[j].order_id} for j in range(len(ans))],
                                 "assign_time": str(now().isoformat()) + 'Z'}, status=status.HTTP_200_OK)
            else:
    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=False):
            ...
            if not courier.last_order:
                order.finish_time = (complete_time - new.assign_time).total_seconds()
            else:
                order.finish_time = (complete_time - courier.last_order).total_seconds()
            order.save()
